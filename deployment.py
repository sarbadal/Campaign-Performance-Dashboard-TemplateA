...
def sync_static_files(static_dir: Path, bucket_name: str, dry_run: bool = False) -> None:
...
def build_runtime_env_vars(deployment_env: dict[str, str]) -> dict[str, str]:
    # Push every key from the provided .env into Cloud Functions runtime config.
...
def main() -> int:
    try:
        ...
        bucket_name = resolve_deployment_value(
            args.bucket_name,
            deployment_env,
            ["DEPLOY_BUCKET_NAME", "BUCKET_NAME", "STATIC_BUCKET"],
        )
        if not bucket_name:
            raise RuntimeError(
                "Missing bucket name. Set --bucket-name or define DEPLOY_BUCKET_NAME (or BUCKET_NAME/STATIC_BUCKET) in .env"
            )
        ...
        create_bucket_if_missing(bucket_name, project_id, bucket_location, dry_run=args.dry_run)
        ensure_public_object_access(bucket_name, dry_run=args.dry_run)
        sync_static_files(static_dir, bucket_name, dry_run=args.dry_run)
